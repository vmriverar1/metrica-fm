/**
 * API Newsletter Migration - Endpoint para ejecutar migración
 * Migración desde JSON a Firestore usando arquitectura unificada
 */

import { NextRequest, NextResponse } from 'next/server';
import { newsletterMigrator, NewsletterJSONReader } from '@/lib/migration/newsletter-migration';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🚀 Starting Newsletter migration (${migrationMode}, dryRun: ${dryRun})`);

    let result;

    if (migrationMode === 'legacy') {
      // Migración desde formato JSON legacy
      result = await NewsletterJSONReader.migrateFromLegacyJSON(jsonFilePath, dryRun);
    } else {
      // Migración usando datos mock (para testing)
      result = await newsletterMigrator.migrateNewsletterSystem(undefined, undefined, undefined, dryRun);
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      message: 'Newsletter migration service ready'
    } as APIResponse);
